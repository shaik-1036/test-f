...
function UserDashboard() {
  ...
  const fetchMessages = async () => {
    try {
      ...
      const newUnread = newMessages.length - prevMessageCount;
      if (newUnread > 0 && !hasNotified) {
        setUnreadMessages(unreadMessages + newUnread);
        toast.info(`You have ${newUnread} new message(s)!`);
        setHasNotified(true);
      }
    } catch (err) {
  ...
  const handleNotificationClick = () => {
    ...
    // Scroll to messages or highlight (handled by UI focus)
  };
